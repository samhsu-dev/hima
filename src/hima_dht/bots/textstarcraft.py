...
CRITICAL_BUILDING_DISTANCE = 3 
DEFEND_UNIT_COUNT = 5
DEFEND_UNIT_ASSIGN_INTERVAL = 200
MIN_DISTANCE = 3
BUILDING_DISTANCE = 7
...
class ChatBot_SingleTurn:
    ...
    def __init__(self, model_name, api_key, api_base, system_prompt, example_prompt, temperature):
        ...
        self.chat = OpenAI(api_key=api_key)
        self.messages = []
...
class ActionExtractor:
    ...
    def extract_and_search_actions(self, decision):
...
class TextStarCraft(BotAI):
    def __init__(self, args, hep=False):
        if hep:
            self.system_prompt = HEP_TEXTSTARCRAFT_PROMPT['system']
            example_input_prompt = HEP_TEXTSTARCRAFT_PROMPT['input']
            example_output_prompt = HEP_TEXTSTARCRAFT_PROMPT['output']
        else:
            self.system_prompt = TEXTSTARCRAFT_PROMPT['system']
            example_input_prompt = TEXTSTARCRAFT_PROMPT['input']
            example_output_prompt = TEXTSTARCRAFT_PROMPT['output']

        self.model_name = args.LLM_api_text
        self.api_key = args.LLM_api_key
        self.api_base = "https://api.openai.com/v1"
        self.temperature = args.temperature
        self.raw_observation_file_name = "raw_observation.json"
        self.L1_observation_file_name = "L1_observations.json"
        self.commander_file_name = 'commander.json'
        ...
        self.example_prompt = [example_input_prompt, example_output_prompt]
        ...
        self.action_mix_rate = 0.5
        self.last_k = 5
        self.action_description = ActionDescriptions('Protoss')
        self.action_dict = self.get_action_dict()
        self.action_dict2 = self.action_description.action_descriptions
    ...
    def get_empty_action_idx(self):
        flat_dict = {}
        for key, value in self.action_dict2.items():
    ...
    def extract_actions_from_command(self, command):
        def extract_actions_from_command2(command, action_extractor, empty_idx):
            ...
            for decision in extracted_decisions:
                ids, actions = action_extractor.extract_and_search_actions(decision)
        ...
        self.action_extractor = ActionExtractor(self.action_dict2)
        empty_idx = self.action_description.empty_action_id
        action_ids, valid_actions = extract_actions_from_command2(command, action_extractor=self.action_extractor,
                                                                 empty_idx=empty_idx)
        return action_ids, valid_actions
    ...
    def action(self, observation):
        ...
        if isinstance(raw_observation, dict):
            L1_observation = self.generate_summarize_L1(raw_observation)
    ...
    def generate_summarize_L1(self, information):
    ...
    async def on_step(self, iteration: int):
        ...
        observation = {
            ...
            'action_executed': copy.deepcopy(self.action_dict[self.acti]),
            'action_failures': copy.deepcopy(self.temp_failure_list)
        }
        self.temp_failure_list.clear()
        self.acti = self.action(observation)

        # 调用相应的动作处理函数
        method_name = f'handle_action_{self.acti}'
    ...
    async def on_end(self, game_result):
        self._save_data_to_file({"time": self.time_formatted, "summary": self.state.score.summary}, "metric.json")
